...
export function Macbook14Model(props: React.JSX.IntrinsicElements['group'] & { color?: string }) {
  const { nodes, materials, scene } = useGLTF('/models/macbook-14-transformed.glb') as unknown as MyGLTFResult
  ...
  useEffect(() => {
    ...
    scene.traverse((child) => {
      if (!(child as THREE.Mesh<THREE.BufferGeometry, THREE.MeshStandardMaterial>).isMesh) return;
      ...
      (child as THREE.Mesh<THREE.BufferGeometry, THREE.MeshStandardMaterial>).material.color.set(new THREE.Color(props.color));
      (child as THREE.Mesh<THREE.BufferGeometry, THREE.MeshStandardMaterial>).material.needsUpdate = true;
    });
  }, [props.color, scene]);
